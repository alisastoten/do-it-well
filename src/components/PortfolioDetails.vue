<template>
    <div class="flex flex-col md:flex-row items-center justify-center mt-10">
      <img :src="job.img" alt="Job Image" class="w-7/8 md:w-3/10 rounded-lg" />
      <div class="mx-10 mt-5 text-white w-7/8 md:w-4/10">
  ...
  import(`../components/Job-${jobFilename}.vue`).then(module => jobComponent.value = module.default);

  const job = ref(jobs.find(job => job.filename === jobFilename));
  </script>
